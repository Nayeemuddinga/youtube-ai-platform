...
async def optimize_video_seo(seo_input: SEOInput, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    system_prompt = """You are a YouTube SEO expert. Return ONLY valid JSON matching this exact structure:
{"titles": [{"title": "string (max 60 chars)", "score": 1-10, "reason": "string"}], "description": "string", "tags": ["string"], "hashtags": ["#string"]}
