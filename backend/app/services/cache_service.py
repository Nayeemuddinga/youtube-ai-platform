...
class CacheService:
    ...
    def _generate_key(self, prefix: str, data: dict) -> str:
        return f"{prefix}:{hashlib.sha256(json.dumps(data, sort_keys=True).encode()).hexdigest()[:12]}"
    ...
    async def set(self, prefix: str, data: dict, value: dict, ttl: int = None) -> bool:
        try:
            await self.redis.setex(self._generate_key(prefix, data), ttl or settings.CACHE_TTL_SECONDS, json.dumps(value))
            return True
        except:
            return False
